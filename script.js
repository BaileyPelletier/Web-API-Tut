...
// Create a request variable and assign a XMLHTTPRequest object to it
...
request.onload = function () {
  ...
  if (request.status >= 200 && request.status < 400) {
    data.forEach(movie => {
      ...
      p.textContent = movie.description + '...'; // End with ellipse

      // append card to container
      container.appendChild(card);
      // each card will contain a h1 and p
      card.appendChild(h1);
      card.appendChild(p);
